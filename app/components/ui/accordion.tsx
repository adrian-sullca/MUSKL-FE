...
>(({ className, ...props }, ref) => (
  <AccordionPrimitive.Item
    ref={ref}
    className={cn("border-b border-primary-color-600/60", className)}
    {...props}
  />
));
...
>(({ className, children, hideIcon = false, ...props }, ref) => (
  <AccordionPrimitive.Header className="flex">
    ...
    >
      {children}
      {!hideIcon && (
        <ChevronDown className="h-4 w-4 shrink-0 text-primary-color-300 transition-transform duration-200" />
      )}
    </AccordionPrimitive.Trigger>
